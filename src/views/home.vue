<template>
  <div>
    ...
    <button @click="decrement()">vuex  -</button><br/><hr>
    <button @click="testAxiosGet">axion get</button><br/><hr>
    <button @click="clickSet">Vue.set(添加属性)</button>
    <button @click="clickDelete">Vue.delete(删除属性)</button>
    <h2>请发表对于VUE的看法</h2>
    ...
    methods: {
      getChildren(){
        ...
        for(let i=0;i<this.$children.length;i++){
          console.log(this.$children[i]);//输出子组件的msg数据；
        }
        this.$children[0].add('白飞飞', '遇见沈浪。。。')
      },
      clickSet(){
        this.$set(this.infor, 'b', 'set-String-b')
      },
      clickDelete(){
        this.$delete(this.infor, 'b')
      },
      testAxiosGet(){
